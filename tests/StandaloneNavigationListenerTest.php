...
namespace {
    ...
    use OCA\AdRoom\Listener\StandaloneNavigationListener; use OCA\LocalBase\Service\StandaloneAppNavigationService; use OCP\App\IAppManager; use OCP\INavigationManager; use OCP\IURLGenerator; use OCP\IUser; use OCP\IUserSession; use OCP\Navigation\Events\LoadAdditionalEntriesEvent;
    ...
    if (($entry['id'] ?? '') !== 'adroom' || ($entry['name'] ?? '') !== 'Räume' || ($entry['href'] ?? '') !== 'adroom.page.index') throw new RuntimeException('Standalone-Raumnavigation fehlt.');
    echo "AD Raum standalone navigation test passed\n";
}
